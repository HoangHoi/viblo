...
CoapLayer::CoapLayer(const CoapLayer& orig) {
}

CoapLayer::~CoapLayer() {
}

int CoapLayer::parse(coap_packet_t *pkt, const uint8_t *buf, size_t buflen) {

    int rc;
    if (0 != (rc = parseHeader(&pkt->hdr, buf, buflen))) {
        return rc;
    }

    if (0 != (rc = parseToken(&pkt->tok, &pkt->hdr, buf, buflen))) {
        return rc;
    }

    pkt->numopts = MAXOPT;
    if (0 != (rc = parseOptionsAndPayload(pkt->opts, &(pkt->numopts), &(pkt->payload), &pkt->hdr, buf, buflen))) {
        return rc;
    }
...
int CoapLayer::parseHeader(coap_header_t *hdr, const uint8_t *buf, size_t buflen) {
    if (buflen < 4) {
...
int CoapLayer::parseToken(coap_buffer_t *tokbuf, const coap_header_t *hdr, const uint8_t *buf, size_t buflen) {
    if (hdr->tkl == 0) {
        tokbuf->p = NULL;
        tokbuf->len = 0;
...
int CoapLayer::parseOption(coap_option_t *option, uint16_t *running_delta, const uint8_t **buf, size_t buflen) {
    const uint8_t *p = *buf;
    uint8_t headlen = 1;
    uint16_t len, delta;
...
int CoapLayer::parseOptionsAndPayload(coap_option_t *options, uint8_t *numOptions, coap_buffer_t *payload, const coap_header_t *hdr, const uint8_t *buf, size_t buflen) {
...
int CoapLayer::make(coap_method_t method, coap_packet_t *pkt, String uri, const uint8_t *payload, size_t payloadlen, uint16_t msgId, const coap_buffer_t* tok) {

}

int CoapLayer::bufferToString(char *strbuf, size_t strbuflen, const coap_buffer_t *buf) {
...
const coap_option_t * CoapLayer::findOptions(const coap_packet_t *pkt, uint8_t num, uint8_t *count) {
    size_t i;
...
int CoapLayer::build(uint8_t *buf, size_t *buflen, const coap_packet_t *pkt) {
    size_t opts_len = 0;
    size_t i;
    uint8_t *p;
    ...
    for (i = 0; i < pkt->numopts; i++) {
        ...
        optionNibble(optDelta, &delta);
        optionNibble((uint32_t) pkt->opts[i].buf.len, &len);
...
int CoapLayer::makeResponse(coap_rw_buffer_t *scratch, coap_packet_t *pkt, const uint8_t *content, size_t content_len, uint8_t msgid_hi, uint8_t msgid_lo, const coap_buffer_t* tok, coap_responsecode_t rspcode, coap_content_type_t content_type) {
    pkt->hdr.ver = 0x01;
...
void CoapLayer::optionNibble(uint32_t value, uint8_t *nibble) {
    if (value < 13) {
        *nibble = (0xFF & value);
