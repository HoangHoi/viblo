...
uint16_t IPv6Stack::udpDataAvailable() {
    return IPv6Stack::udp_data_length_left;
}

uint8_t IPv6Stack::isCoapPacket() {
    if (IPv6Stack::udp_data[2] == 0x16 && IPv6Stack::udp_data[3] == 0x33) {
        return 1;
    }
...
void IPv6Stack::getCoapData(uint8_t* buffer) {
    memcpy(buffer, &IPv6Stack::udp_data[8], IPv6Stack::udp_data_length - 8);
}

uint16_t IPv6Stack::getCoapDataLength() {
    return IPv6Stack::udp_data_length - 8;
}
...
void IPv6Stack::getUdpData(uint8_t* buffer) {
    memcpy(buffer, IPv6Stack::udp_data, IPv6Stack::udp_data_length);
}

uint16_t IPv6Stack::getUdpSenderPort() {
    uint16_t hport = IPv6Stack::udp_data[0];
    uint16_t lport = IPv6Stack::udp_data[1];
    return hport << 8 | lport;
}
